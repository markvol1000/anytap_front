...
## 프로젝트 절대 제약 사항 (Strict Mandates)

> **아래 4대 원칙은 모든 작업에서 타협 없이 최우선으로 준수해야 합니다.**

1. **Mock 데이터 절대 금지**: 프로덕션 및 실서버/개발 환경 API 연동 시 Mock 데이터, 하드코딩된 가상 목업 사용을 전면 금지한다. 실제 백엔드 API 및 외부 연동사(Wasabi, Cregis 등)의 실데이터만 사용한다.
2. **Fallback 가상 데이터 금지**: API 응답이 없거나 필드 누락 시 임의로 생성한 가상/더미 데이터(dummy/synthetic fallback)를 UI나 DB에 채워 넣지 않는다. 데이터가 없으면 '데이터 없음' 상태(null, 빈 배열, 빈 문자열, '—')로 투명하게 표시한다.
3. **통화(Currency) 강제 기본값(USD/USDT) 주입 절대 금지**: 통화 값이 null이거나 비어있을 경우, 강제로 'USD'나 'USDT'를 기본값(`|| 'USDT'`, `|| 'USD'`)으로 주입하는 로직을 일체 금지한다. 데이터 원본(웹훅/DB/API)의 통화 필드 그대로 처리하며, 없을 경우 null 또는 빈 값으로 유지한다.
4. **Git 커밋/푸시 및 서버 배포 임의 수행 절대 금지**: AI 어시스턴트 및 에이전트는 `git commit`, `git push`, 그리고 실서버/운영/개발 환경 배포 스크립트 실행(예: `deploy_amplify.ps1`, `prd_deploy.ps1`, `dev_deploy.ps1` 등)을 일체 직접 수행하지 않는다. 모든 커밋, 푸시, 배포는 오직 사용자가 직접 검토하고 수동으로 실행한다.
...
| **백엔드** | Spring Boot 백엔드 연동 (`/api/*`, `/admin/*`) |
| **카드 API** | **Wasabi API** 사용 (카드 발급·관리, 실데이터 연동) |
| **결제 API** | **Cregis API** 사용 (온체인 입출금, 실데이터 연동) |
